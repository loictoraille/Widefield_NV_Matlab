...
LIGHT = 1; % lumi�re allum�e pendant les proc�dures d'alignement PerformAlignPiezo % value set in the Camera Panel
...
elseif strcmp(SaveMode,'h5')
    ...
    % Save without additional options
    saveArgs = varList;
end
...
if i_scan == 1 && ~TestWithoutHardware && AutoAlignPiezo
    ...
    CheckMaxAndWriteNI(AcqParameters.PiezoX, AcqParameters.PiezoY, AcqParameters.PiezoZ, Tension_Light)
    if strcmp(CameraType,'Andor')
        EndAcqCamera();
        [I,ISize,AOI] = PrepareCamera();
    end
...
NPerm = 1; % d�coupage de la Width_MHz totale en segments pour �viter des �carts de fr�quence trop grands entre les pics
    ...
    for ii=1:NPoints %Loop on number of RF points
        ...
        if ~TestWithoutHardware
            if RANDOM == 1
                ...
                pause(0.05/NPerm); % plus il y a de permutations, moins la pause a besoin d'�tre longue
            else
                if mod(Acc,2) == 1 % montant puis descendant une acq sur deux pour �viter l'erreur d� au retour au premier point de la rampe
...
%         end
        ...
        %     Ftot = Ftot(2:end-1);
        %     M = M(:,:,2:end-1); %% this line takes a lot of time for no
        %     significant gain
        % end            
        drawnow;%Update GUI
        % if DelEx
