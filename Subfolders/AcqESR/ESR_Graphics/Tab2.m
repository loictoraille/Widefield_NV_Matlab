...
%% Pixel Calibration
bg = uibuttongroup('Parent',tab2,'units','normalized',...
                  'Position',[0.6 0.15 0.15 0.11],'tag','calibunit',...
                  'SelectionChangedFcn',@CalibUnit);              
              
uicontrol('Parent',bg,'Style','text','units','normalized',...
    'FontSize',20,'FontWeight','bold','HorizontalAlignment','center','Position',...
    [0.01 0.65 0.98 0.3],'String','Pixel Calibration');
              
r1 = uicontrol(bg,'Style',...
                  'radiobutton','units','normalized',...
                  'String','pixel','FontSize',14,...
                  'Position',[0.18 0.1 0.25 0.2]);
              
r2 = uicontrol(bg,'Style','radiobutton','units','normalized',...
                  'String','microns','FontSize',14,...
                  'Position',[0.5 0.1 0.35 0.2]);
...
hp4 = uipanel('Parent',tab2,'Title','Camera','FontSize',12,'FontWeight','bold',...
    'BackgroundColor','white','Position',[0.01 0.28 .55 .7]);
axCam=axes('Parent',hp4,'tag','Axes_Camera',...
...
%%%%%%%%%
%Start acquisition
AcqCont=uicontrol('Parent',tab2,'Style', 'togglebutton', 'String', 'ON','units','normalized','tag','acqcont',...
    'Position', [0.01 0.18 0.15 0.08],'FontSize',20,'ForegroundColor',[0,0,0],...
    'Value',0,'Callback',@AcqContinue);
%Stop acquisition
StopCam=uicontrol('Parent',tab2,'Style', 'togglebutton', 'String', 'OFF','units','normalized','tag','stopcam',...
    'Position', [0.2 0.18 0.15 0.08],'FontSize',20,'ForegroundColor',[0,0,0],...
    'Value',0);
...
%%%%%%%%%
%% Exposure time
Sld_Exp=uicontrol('Parent',tab2,'Style','slider','Min',1,'Max',100,'Value',50,...
    'tag','sldexp','units','normalized','Position', [0.73 0.75 0.2 0.05],'Callback',@SldExp);
expmin=uicontrol('Parent',tab2,'Style','text','string',1,'FontSize',10,...
    'tag','expmin','units','normalized','Position', [0.67 0.76 0.05 0.03],'HorizontalAlignment','right');
expmax=uicontrol('Parent',tab2,'Style','text','string',100,'FontSize',10,...
    'tag','expmax','units','normalized','Position', [0.94 0.76 0.05 0.03],'HorizontalAlignment','left');
exptxt=uicontrol('Parent',tab2,'Style','text','string','Exposure','FontSize',15,...
    'tag','exptext','units','normalized','Position', [0.73 0.82 0.2 0.03]);
...
%% Frame Rate
Sld_FrameRate=uicontrol('Parent',tab2,'Style','slider','Min',1,'Max',100,'Value',50,...
    'tag','sldframe','units','normalized','Position', [0.73 0.55 0.2 0.05],'Callback',@SldFra);
framin=uicontrol('Parent',tab2,'Style','text','string',1,'FontSize',10,...
    'tag','framin','units','normalized','Position', [0.67 0.56 0.05 0.03],'HorizontalAlignment','right');
framax=uicontrol('Parent',tab2,'Style','text','string',100,'FontSize',10,...
    'tag','framax','units','normalized','Position', [0.94 0.56 0.05 0.03],'HorizontalAlignment','left');
fratxt=uicontrol('Parent',tab2,'Style','text','string','Frame Rate','FontSize',15,...
    'tag','fratext','units','normalized','Position', [0.73 0.62 0.2 0.03]);
...
%% PixelClock
Sld_PixClock=uicontrol('Parent',tab2,'Style','slider','Min',1,'Max',100,'Value',50,...
    'tag','sldpix','units','normalized','Position', [0.73 0.35 0.2 0.05],'Callback',@SldPix);
pixmin=uicontrol('Parent',tab2,'Style','text','string',1,'FontSize',10,...
    'tag','pixmin','units','normalized','Position', [0.67 0.36 0.05 0.03],'HorizontalAlignment','right');
pixmax=uicontrol('Parent',tab2,'Style','text','string',100,'FontSize',10,...
    'tag','pixmax','units','normalized','Position', [0.94 0.36 0.05 0.03],'HorizontalAlignment','left');
pixtxt=uicontrol('Parent',tab2,'Style','text','string','Pixel Clock','FontSize',15,...
    'tag','pixtext','units','normalized','Position', [0.73 0.42 0.2 0.03]);
...
%% Piezo control
uicontrol('Parent',tab2,'Style','text','units','normalized','FontSize',16,'FontWeight','bold','HorizontalAlignment','left','Position',...
    [0.593 0.75 0.1 0.03],'String','Piezo Control');
panel = uipanel('Parent',tab2,'Position',[0.58, 0.6, 0.1, 0.15]);

uicontrol('Parent', panel, 'Style', 'togglebutton','tag','initpiezo','ForegroundColor',[0,0,0],'Value',0, 'String', 'Init Piezo', 'FontSize',12,'units','normalized',...
'Tooltip','Initialize piezo values at 0 then input user-defined values','Position', [0.1, 0.77, 0.8, 0.2], 'Callback', @InitPiezo);
...
uicontrol('Parent',panel, 'Style', 'edit','FontSize',12,'units','normalized','FontSize',FS2, 'Position', PositionRow2 + [0, 0, 0, 0],'tag','piezoRange',  'String',num2str(AcqParameters.PiezoRange),'Callback',@UpdateAcqParam);
...
uicontrol('Parent',panel, 'Style', 'edit','FontSize',12,'units','normalized','FontSize',FS2, 'Position', PositionRow2 + [0.3, 0, 0, 0],'tag','piezoSteps',  'String',num2str(AcqParameters.PiezoSteps),'Callback',@UpdateAcqParam);
uicontrol('Parent',panel, 'Style', 'togglebutton','tag','lightpiezo','FontSize',12,'units','normalized','FontSize',FS2, 'String', 'Light', 'Position', PositionRow2 + [0.56, 0.2, 0.05, 0],'HorizontalAlignment','center','Callback',@LightPiezo);
uicontrol('Parent',panel, 'Style', 'edit','FontSize',12,'units','normalized','FontSize',FS2, 'Position', PositionRow2 + [0.6, 0, 0, 0],'tag','piezoLightValue',  'String',num2str(AcqParameters.PiezoLight),'Callback',@UpdatePiezo);


%% Calib Piezo
calibPanelTitle = uicontrol('Parent',tab2,'Style','text','units','normalized','FontSize',16,'FontWeight','bold','HorizontalAlignment','left','Position',...
    [0.593 0.565 0.1 0.03],'String','Calib (um/10V)');
calibPanel = uipanel('Parent',tab2,'Position',[0.61, 0.465, 0.05, 0.1]);
...
uicontrol('Parent',tab2,'Style','text','tag','cameratype','units','normalized',...
    'FontSize',16,'FontWeight','bold','HorizontalAlignment','right','Position',...
    [0.66 0.93 0.2 0.03],'String','Camera type');

uicontrol('Parent',tab2,'Style', 'text', 'String', sprintf('TEST WITHOUT\n HARDWARE'),'ForegroundColor','red','units','normalized','tag','testwohar2',...
    'Position', [0.795 0.87 0.085 0.055],'FontSize',15,'FontWeight','bold','VISIBLE',valuetestwohar);